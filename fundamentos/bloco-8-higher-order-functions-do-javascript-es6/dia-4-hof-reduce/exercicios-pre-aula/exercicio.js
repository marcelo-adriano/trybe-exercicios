...
const estudantes = [
  ...
  */
  ...
  const maior = (maiorNumero, outroNumero) => maiorNumero.nota > outroNumero.nota ? maiorNumero : outroNumero;

  function melhorMateria() {
      return estudantes.map((estudante) => {
          let resposta = { };
          resposta.name = estudante.nome;
          resposta.materia = estudante.materias.reduce(maior).name;
          return resposta;
      })
  }

  assert.deepStrictEqual(melhorMateria(), resultadoEsperado);