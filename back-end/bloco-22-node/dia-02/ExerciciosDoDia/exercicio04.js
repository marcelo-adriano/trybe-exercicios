...
function funcaoUm() {
  return readFile('./simpsons.json')
    .then((rawData) => JSON.parse(rawData))
    .then((json) => json.forEach(({ id, name }) => {
      console.log(`${id} - ${name}`);
    }));
}

async function funcaoDois(id) {
  const rawData = await readFile('./simpsons.json');
  const json = await JSON.parse(rawData);
  return new Promise((resolve, reject) => {
    const idFind = json.find((person) => person.id === id.toString());
    if (!idFind) reject('Personagem não existe');
    else resolve(`${idFind.id} - ${idFind.name}`);
  });
}
...
function funcaoTres() {
  return readFile('./simpsons.json')
    .then((rawData) => JSON.parse(rawData))
    .then((json) => {
      writeFile('./simpsons2.json', JSON.stringify(json.filter(({ id }) => id !== '10' && id !== '6')));
    }); 
}
...
function funcaoQuatro() {
  return readFile('./simpsons.json')
    .then((rawData) => JSON.parse(rawData))
    .then((json) => {
      writeFile('./simpsonFamily.json', JSON.stringify(json.filter(({ id }) => /^[1-4]$/.test(id))));
    }); 
}
